...
// this is a variable to signal calling program for error, also useful in running tests
int isOkay = 0;

int main(int argc, char* argv[]) {
	
	fSignal = fopen("signal.txt", "w");
	if (fSignal == NULL) {
		printf("cant open signal file\n");
		return 0;
	}
	...
	printf("Opening viper file: %slol\n", argv[1]);
    FILE *file = fopen(inputFile,"r");
    
    if (file == NULL) {
    	printf("error opening file\n");
		isOkay = 1;
        fprintf(fSignal, "%d", isOkay);
		fclose(fSignal);
	}
	...
	// isOkay is the signal variable to the calling program when error occurs
	// so it won't continue to parsing
//	printf("Yow");
	return isOkay;
	...
	void lexicalAnalyzer(char file[]) {
		...
		if(hasError != 0) {
			printf("\n\n...\n\n\nINVALID LEXEME DETECTED. %d ERRORS FOUND.\nCANNOT CONTINUE TO PARSING PHASE.\n");
			isOkay = 1;
			fprintf(fSignal, "%d", isOkay);
			fclose(fSignal);
		}else{
